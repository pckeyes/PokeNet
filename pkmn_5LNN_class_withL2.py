...
data_path = "/home/ubuntu/PokeNet/type_5LNN_L2_output/"
#Import and normalize data
#currently set to load type as Y vector
_, X, Y_type, _, _, _, _, _, _, _ = pkmn_data.pkmn_load_data_vec(9791)
X = X/255
...
dev_len = (X_shuffled.shape[1] - train_end_index)/2 #use 10% of data for dev and 10% for test
dev_end_index = int(train_end_index + dev_len)
X_dev = X_shuffled[:,train_end_index:dev_end_index]
X_test = X_shuffled[:,dev_end_index:]
n_x = X_train.shape[0]
Y_train = Y_type_vectorized_shuffled[:,:train_end_index]
Y_dev = Y_type_vectorized_shuffled[:,train_end_index:dev_end_index]
Y_test = Y_type_vectorized_shuffled[:,dev_end_index:]
...
def compute_cost(Z5, Y, parameters, beta):
    W1 = parameters['W1']
    W2 = parameters['W2']
    W3 = parameters['W3']
    W4 = parameters['W4']
    ...
    L2_regularization_cost = beta * (tf.nn.l2_loss(W1) + tf.nn.l2_loss(W2) + tf.nn.l2_loss(W3) + tf.nn.l2_loss(W4) + tf.nn.l2_loss(W5))
    cost = cost + L2_regularization_cost
    return cost

def model(X_train, Y_train, X_dev, Y_dev, X_test, Y_test, beta, learning_rate = 0.0005, num_epochs = 150,  print_cost = True):
    ...
    #compute cost
    cost = compute_cost(Z5, Y, parameters, beta)
...
beta = 0.001
start = time.time()
train_accuracy, dev_accuracy, test_accuracy, parameters, costs = model(X_train, Y_train, X_dev, Y_dev, X_test, Y_test, beta, lr, num_epochs)
end = time.time()
duration = end - start

# Save the performance specs as a .txt file
save_file = open(data_path + str(lr) + "_" + str(num_epochs) + "_" + str(beta) + ".txt", "w+")
